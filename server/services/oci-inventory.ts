...
export class OCIInventoryService {
  ...
  constructor(credentials: OCICredentials) {
    ...
    // Use SimpleAuthenticationDetailsProvider with region for older SDK version
    this.provider = new common.SimpleAuthenticationDetailsProvider(
      ...
      credentials.region // Try with region for older SDK
    );
  }
  ...
  async validateCredentials(): Promise<boolean> {
    try {
      // Test credentials by making a simple API call to list compartments
      const identityClient = new identity.IdentityClient({ 
        authenticationDetailsProvider: this.provider
      });
      const listCompartmentsRequest: identity.requests.ListCompartmentsRequest = {
      ...
      await identityClient.listCompartments(listCompartmentsRequest);
      return true;
    } catch (error) {
